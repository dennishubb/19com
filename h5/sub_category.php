...
?>
    <div class="main_container">
        <div class="body_container">
            ...
            <?php
            $data = httpGet(CURL_API_URL . '/service/news.php?action=get_category_news_pagination&limit=5&category_id='.$cid.'&sub_category_id='.$scid.'&type=latest_news');
            $data = json_decode($data, true);
            ...
            <session>
                <div class="session_block">
                    <div class="session_block_title">
                        即时新闻 <?php if($total_page > 1){ ?><span style="float:right; margin-right:11px;" onclick='window.location.href="more.php?category=<?php echo $cid; ?>&sub_category=<?php echo $scid; ?>&type=latest_news"'>更多▶</span><?php } ?>
            ...
            <?php } ?>
            <?php
            $match_analytics = httpGet(CURL_API_URL . '/service/news.php?action=get_category_news_pagination&limit=5&category_id='.$cid.'&sub_category_id='.$scid.'&type=match_analytics');
            $match_analytics = json_decode($match_analytics, true);
			$total_page 	 = $match_analytics['total_page'];
			$match_analytics = $match_analytics['data'];
            ?>
            <?php if(count($match_analytics) > 0){?>
            <session>
                <div class="session_block">
                    <div class="session_block_title">
                        赛事分析 <?php if($total_page > 1){ ?><span style="float:right; margin-right:11px;" onclick='window.location.href="more.php?category=<?php echo $cid; ?>&sub_category=<?php echo $scid; ?>&type=match_analytics"'>更多▶</span> <?php } ?>
                        <!--<a href="#" class="more">更多 <img src="img/arrow.png"></a>-->
                    </div>
                    <div class="break_line"></div>
            ...
            <?php } ?>
            <?php
            $team_intro = httpGet(CURL_API_URL . '/service/news.php?action=get_category_news_pagination&limit=5&category_id='.$cid.'&sub_category_id='.$scid.'&type=team_intro');
            $team_intro = json_decode($team_intro, true);
			$total_page = $team_intro['total_page'];
			$team_intro = $team_intro['data'];
            ?>
            <?php if(count($team_intro) > 0){?>
            <session>
                <div class="session_block">
                    <div class="session_block_title">
                        球队介绍 <?php if($total_page > 1){ ?><span style="float:right; margin-right:11px;" onclick='window.location.href="more.php?category=<?php echo $cid; ?>&sub_category=<?php echo $scid; ?>&type=team_intro"'>更多▶</span><?php } ?>
                        <!-- <a href="#" class="more">更多 <img src="img/arrow.png"></a> -->
                    </div>
                    <div class="break_line"></div>
