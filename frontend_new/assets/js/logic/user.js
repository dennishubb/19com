...
function getAdminList() {
    ...
    datatable = $('#admin_datatable').KTDatatable({
        ...
        data: {
            ...
            source: {
                read: {
                    url: link + 'api/cn/user?sort[field]= user.created_at &sort[sort]=desc', //?search[type]=Member
                    headers: getHeaders(),
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (data, i) {////console.log(data);
                    output = '' +
                        ...
                        '<div>' +
                        '<a href="admin-edit.html?id=' + data.id + ' "class="kt-user-card-v2__name" style="color:#5867dd" >' + data.username + '</a>' +
            ...
            },
			{
                ...
                template: function (row) {
                    ...
                    var output = '' +
                        '<div class="kt-user-card-v2">' + row.created_at +
                        '</div>';
        ...
        data: {
            ...
            source: {
                read: {
                    ...
                    method: 'GET',
                    params: {
                        //filter:[{field:'user.type', value:'Member', operator:'='}, {field:'deleted', value:'0', operator:'='}]
						search:{type:'Member', 'deleted':0}
                    }
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (data, i) {////console.log(data);
                    output = '' +
                        ...
                        '<div>' +
                        '<a href="member-edit.html?id=' + data.id + ' "class="kt-user-card-v2__name" style="color:#5867dd" >' + data.username + '</a>' +
            ...
            },
            {
                ...
                template: function (data, i) {
                    ...
                    var output = '' +
                        '<div class="kt-user-card-v2">' + parseInt(data.points) + '</div>';
            ...
            },
            {
                ...
                template: function (data, i) {
                    ...
                    var output = '' +
                        '<div class="kt-user-card-v2">' + parseInt(data.voucher) + '</div>';
...
function searchUserList(type){
	...
	if (type=='member')
		datatablle_id='member_datatable';
	else if (type=='admin')
		datatablle_id='admin_datatable';
	
	$("#"+datatablle_id).KTDatatable().API.params = {};
	
	//filter by keyword
     $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[0][field]', "concat(user.username,user.email,user.name)")
     $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[0][value]', "%"+ SearchKeyword.toLowerCase()+"%")
     $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[0][operator]', "LIKE")
	 
	 $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[1][field]', "user.type")
     $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[1][value]', type)
     $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[1][operator]', "=")
	 
	 if (type=='admin' && $('#kt_form_roleid').val()!='-'){
		 $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[2][field]', "user.role_id")
		 $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[2][value]', $('#kt_form_roleid').val())
		 $("#"+datatablle_id).KTDatatable().setDataSourceParam('filter[2][operator]', "=")
	 }
	 $("#"+datatablle_id).KTDatatable().setDataSourceParam('sort[field]', "user.created_at")
     $("#"+datatablle_id).KTDatatable().setDataSourceParam('sort[sort]', "desc")
	 
	
	//console.log('ni')
     $("#"+datatablle_id).KTDatatable().load();
	 
}

function searchUserBlackWhiteList(){
	var SearchKeyword=$('#SearchKeyword').val();
	...
	if (fullUri.indexOf("member") >= 0)
		user_type='Member';
	else if (fullUri.indexOf("admin") >= 0)
		user_type='Admin';
	
	if (fullUri.indexOf("blacklist") >= 0)
		disabled=1;
	else if (fullUri.indexOf("whitelist") >= 0)
		disabled=0;
	
	$(".kt-datatable").KTDatatable().API.params = {};
	
	//filter by keyword
     $(".kt-datatable").KTDatatable().setDataSourceParam('filter[0][field]', "concat(user.username,user.email,user.name)")
	 ...
	 $(".kt-datatable").KTDatatable().setDataSourceParam('filter[1][field]', "user.type")
     $(".kt-datatable").KTDatatable().setDataSourceParam('filter[1][value]', user_type)
     $(".kt-datatable").KTDatatable().setDataSourceParam('filter[1][operator]', "=")
	 
	 $(".kt-datatable").KTDatatable().setDataSourceParam('filter[2][field]', "user.disabled")
     $(".kt-datatable").KTDatatable().setDataSourceParam('filter[2][value]', disabled)
     $(".kt-datatable").KTDatatable().setDataSourceParam('filter[2][operator]', "=")
	 
	 $(".kt-datatable").KTDatatable().setDataSourceParam('sort[field]', "user.id")
...
function getBlackWhitelist(type,user_type) {
    ...
    if (type == 'whitelist') {
		...
		datatable_id='whitelist_datatable';
    } else if (type == 'blacklist') {
        disabled = 1;
        var redirect_uri = pre_redirect_uri+'-blacklist.html';
		datatable_id='blacklist_datatable';
    }
	
    datatable = $('#'+datatable_id).KTDatatable({
        // datasource definition
        data: {
            type: 'remote',
            source: {
                read: {
                    url: link + 'api/cn/user?sort[field]= user.id &sort[sort]=desc', //?search[type]=Member
                    // url: 'https://keenthemes.com/metronic/tools/preview/api/datatables/demos/client.php',
                    //url: 'http://test.19com.backend:5280/api/cn/user',
                    method: 'GET',
                    params: {
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (data, i) {////console.log(data);
                    output = '' +
                        ...
                        '<div>' +
                        '<a href="'+next_link+'?from=' + redirect_uri + '&id=' + data.id + ' "class="kt-user-card-v2__name" style="color:#5867dd" >' + data.username + '</a>' +
            ...
            },
            {
                ...
                template: function (data, i) {
                    ...
                    if (data.disabled == 0) {
                        content = '白名单用户';
                        style = 'background-color:green;color:white;padding:1px';
                    } else if (data.disabled == 1) {
                        content = '黑名单用户';
                        style = 'background-color:red;color:white;padding:1px';
                    }

                    var output = '' +
                        '<div class="kt-user-card-v2"><span style=' + style + '>' + content +
                        '</span></div>';
            ...
            },
            {
                ...
                template: function (data, i) {////console.log(data);
                    ...
                    output = '' +
                        ...
                        '<div>' +
                        '<a href="member-rank-edit.html?id=' + data.id + ' "class="kt-user-card-v2__name" style="color:#5867dd" >' + data.name + '</a>' +
...
function user_edit($form, $error_selector = $(".message_output")) {
	...
	validation=true;//current remove all validation  for member and admin edit
    ...
    if (validation == true) {
		...
		$('#form').find(':input').each(function (key, value) {
			...
			if (name=='admin_chg_password')
				new_pw=$(this).val();
			
		});
		
		if (fullUri.indexOf("member-edit") >= 0){
			if (json_form_obj['admin_chg_password'].length>0)
				json_form_obj['password'] = new_pw;
		}
		else if (fullUri.indexOf("admin-edit") >= 0){
			
			if (window.localStorage.admin_username=='admin' && json_form_obj['admin_chg_password'].length>0)
				json_form_obj['password'] = new_pw;
		}
		...
		$.ajax({
			...
			success: function (response, status, xhr) {
				...
				if (obj.code == 1) {
					if (fullUri.indexOf("member-edit") >= 0){
						if (json_form_obj['admin_chg_password'].length>0)
							redirect_msg+=' - 会员密码已更换';
					}
					
					if (obj.redirect) redirect_to($redirect_uri + "&alert-success=" + redirect_msg);
				} else if (obj.code == -1) {
					redirect_to_login();
				} else {
					showAlert(obj.message, "danger", $error_selector);
...
function adduser($form) {
	var fullUri = getCurrentFullUri();
	var validate_field=['username', 'password', 'confirmpassword']; //for member
	
	if (fullUri.indexOf("admin-add") >= 0)
		validate_field=['username' , 'password', 'confirmpassword']; //for admin
