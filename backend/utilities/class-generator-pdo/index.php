...
$output.="
		...
		$"."this->dbc->insert('".$_POST['name']."',$"."data);
...
$output.="
	public function load($".$primary_php_name."){
        ...
        $"."row=$"."this->dbc->select(\"SELECT * FROM ".$_POST['name']." WHERE ".$primary_name." = :id\", array( ':id' => $".$primary_php_name." ));
...
foreach($fields as $field){
	...
	$var_name=strtolower($field);
	$method_name=str_replace(' ', '', ucwords(str_replace('_',' ',$field)) );
	$output.="\t\t\t$"."this->set".$method_name."($"."row[0]->".$field.");\n";
...
$output.="		
		...
		$"."where = array('".$primary_name."' => $"."this->get".$primary_method_name."());
 
		$"."this->dbc->update('".$_POST['name']."',$"."data,$"."where); 
...
$output.="
	public function remove(){
        ...
        $"."data = array('".$primary_name."' => $"."this->get".$primary_method_name."());

        $"."this->dbc->delete('".$_POST['name']."', $"."data );
