...
class Validator {
	 ...
	 */
	public function __construct($new_lang = false) {
		if(!$new_lang) {$lang = $this->language;}
		else {$lang = $new_lang;}
    	...
    	foreach($indexes_array as $index_val) {
			...
			foreach($index_val as $key => $val) {
				...
				if(is_array($passed_data)) {
					$a = 0;
					foreach($passed_data as $passed_key => $single_passed_data) {
						$this->form_val[$index][$passed_key] = is_string($single_passed_data)?trim($single_passed_data):$single_passed_data;
						...
						if($key != 'index' && $key != 'label') {
							...
							if($key == 'min_array' || $key == 'max_array') {
								...
								$index_results[] = $this->validate($validate_type, $validate_val, is_string($single_passed_data)?trim($single_passed_data):$single_passed_data, $label, $hide_err);
							}
						}
						$a++;
					}
					
					if(count($passed_data) == 0){
	...
	private function getIndexVal($index) {
        ...
        elseif(isset($params[$index])) {$index_val = $params[$index];}
	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_val') {
			if((float)$index_val < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = _('minimum value is').' '.$val;
				}
				return false;
			}
			else {return true;}
		...
		if($type == 'max_val') {
			if((float)$index_val > (int)$val) {
				if(!$test) {
					#$this->errors['value'][] = $label;
					$this->custom_error[$label] = _('maximum value is').' '.$val;
				}
				return false;
			}
			else {return true;}
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = _('最少').' '.$val.' '._('位字');
				}
				return false;
			}
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = _('最長').' '.$val.' '._('位字');
				}
				return false;
			}
			else {return true;}
		...
		if($type == 'right_len') {
			if(strlen($index_val) != (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = _('has to be long').' '.$val.' '._('characters');
				}
				return false;
			}
			else {return true;}
		...
		if($type == 'min_array') {
			if(!is_array($index_val) || count($index_val) < $val) {
				if(!$test) {
					...
					$this->custom_error[$label] = _('Select').' '.$val.' '._('options');
				}
				return false;
			}
		}
		...
		if($type == 'max_array' && is_array($index_val)) {
			if(count($index_val) > $val) {
				if(!$test) {
					#$this->errors['array_count'][] = $label;
					$this->custom_error[$label] = _('Maximum').' '.$val.' '._('options allowed');
				}
				return false;
			}
		}
	...
	public function escape_val() {
		...
		foreach($fv as $index=>$val) {
			if(!is_array($val)) { $new_fv[$index] = $val;}
			else {

				foreach($val as $key => $subval) {
					$new_fv[$index][$key] = $subval;
				}
	...
	private function errorTranslate($type, $label_array) {
		...
		switch($type) {
			case 'required' :
				(count($label_array) > 1) ? $message .= _('是必填') : $message .= _('是必填');
				break;

			case 'wrong_int' :
				(count($label_array) > 1) ? $message .= _('are not valid integers') : $message .= _('is not a valid integer');
				break;

			case 'wrong_float' :
				(count($label_array) > 1) ? $message .= _('are not valid floating numbers') : $message .= _('is not a valid floating number');
				break;

			case 'wrong_mail' :
				(count($label_array) > 1) ? $message .= _('are not valid e-mail addresses') : $message .= _('is not a valid e-mail address');
				break;

			case 'wrong_date' :
				(count($label_array) > 1) ? $message .= _('are not valid dates') : $message .= _('is not a valid date');
				break;

			case 'wrong_time' :
				(count($label_array) > 1) ? $message .= _('are not valid times') : $message .= _('is not a valid time');
				break;

			case 'wrong_url' :
				(count($label_array) > 1) ? $message .= _('are not valid urls') : $message .= _('is not a valid url');
				break;

			case 'wrong_hex' :
				(count($label_array) > 1) ? $message .= _('are not valid hexadecimal colors') : $message .= _('is not a valid hexadecimal color');
				break;

			case 'wrong_ip' :
				(count($label_array) > 1) ? $message .= _('are not valid IP addresses') : $message .= _('is not a valid IP address');
				break;

			case 'wrong_zip' :
				(count($label_array) > 1) ? $message .= _('are not valid ZIP codes') : $message .= _('is not a valid ZIP code');
				break;

			case 'wrong_tel' :
				(count($label_array) > 1) ? $message .= _('are not valid telephone numbers') : $message .= _('is not a valid telephone number');
				break;

			case 'wrong_type' : $message .= _('invalid data inserted');
				break;

			case 'allowed' :
				(count($label_array) > 1) ? $message .= _('values are not between the allowed') : $message .= _('value is not between the allowed');
				break;

			case 'forbidden' :
				(count($label_array) > 1) ? $message .= _('values are between the forbidden') : $message .= _('values is between the forbidden');
				break;

			case 'not_equal' : $message .= _("the value doesn't match");
				break;

			case 'size' : $message .= _('file size is wrong');
				break;
		}
