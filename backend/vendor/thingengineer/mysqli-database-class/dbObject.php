...
class dbObject {
        ...
        if (property_exists ($this, 'relations') && isset ($this->relations[$name])) {
            ...
            switch ($relationType) {
                ...
                case 'hasmany':
                    ...
                    $obj = new $modelName;
                    $obj->returnType = $this->returnType;
                    return $this->data[$name] = $obj->where($key, isset($this->relations[$name][3]) ? $this->data[$this->relations[$name][3]] : $this->data[$this->primaryKey])->get();
                    break;
                default:
                    break;
            }
	...
	public function updateCustom($data, $where){
		...
		$res = $this->db->update($this->dbTable, $data);
		return $res;
	}
     ...
     * @return dbObject
     */
    private function join ($objectName, $key = null, $joinType = 'LEFT', $primaryKey = null, $alias = "") {
        ...
        if (!$primaryKey){
			if($alias)
				$primaryKey = MysqliDb::$prefix . $alias . "." . $joinObj->primaryKey;
			else
        ...
        foreach ($this->_with as $name => $opts) {
            ...
            if ($relationType == 'hasone') {
                ...
                if (!isset ($data[$table])) {
					...
					if(is_object($this->$name) && $this->$name->joinFields){
						$this->joinFilter($this->$name, $this->$name->data);
					}
                ...
                if (is_array($data[$table]) && $data[$table][$primaryKey] === null) {
                    $data[$name] = null;
                } else {
                    if ($this->returnType == 'Object') {
						//filter here
						...
						if($item->joinFields){
							$this->joinFilter($item, $data[$table]);
						}
    ...
    private function processHasOneWith () {
        ...
        foreach ($this->_with as $name => $opts) {
            ...
            if ($relationType == 'hasone') {
                ...
                $this->join ($modelName, $key, "LEFT", null, array_key_exists(3, $opts)?$opts[3]:"");
            }
        }
    }
	...
	private function joinFilter(&$object, $data){
		
		$newData = array();
		foreach($data as $key => $value){
			if(!in_array($key, $object->joinFields)) continue;
